...
class Data(object):
    def __init__(self, args):
        ...
        if os.path.exists(valid_file):
            ...
            # Split part of the train data as valid data
            valid_ratio = 0.2  # You can adjust the ratio
            num_valid = int(train_mat.nnz * valid_ratio)
            valid_rows = train_mat.row[:num_valid]
            valid_cols = train_mat.col[:num_valid]
            valid_data = train_mat.data[:num_valid]

            # Create sparse matrix for validation data
            valid_mat = sp.coo_matrix((valid_data, (valid_rows, valid_cols)), shape=train_mat.shape)
            # Save as valid.pkl
            with open(valid_file, 'wb') as f:
                pickle.dump(valid_mat, f)

            # Remove validation data from the training data
            train_rows = train_mat.row[num_valid:]
            train_cols = train_mat.col[num_valid:]
            train_data = train_mat.data[num_valid:]

            # Create new training matrix excluding the validation samples
            train_mat = sp.coo_matrix((train_data, (train_rows, train_cols)), shape=train_mat.shape)
